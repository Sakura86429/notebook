	...
	swal({ 
		...
		$.ajax({
			...
			success:function (result) {
				...
				if (result.code == 1) {
					// 提示用户成功
					//最后"success"意思是设置成功的√图标
					swal("","<h3>删除成功！</h3>","success");
					// 执行删除成功之后的DOM操作，deleteDom方法在下面写
					deleteDom(typeId);
				} else {
					// 提示用户失败
...
function deleteDom(typeId) {
	// 1. 设置表格id，通过id属性值，得到表格对象
	var myTable = $("#myTable");
	// 2. 设置tr id和数据库相连id都不一样，得到table元素的子元素tr的数量
	//myTable找它的所有的tr
...
$("#addBtn").click(function (){
	...
	// 清空模态框中文本框与隐藏域的值（清空之后可以isEmpty(typeId)）
	$("#typeId").val("");
	$("#typeName").val("");
	// 清空提示信息
	$("#msg").html("");
	...
	$.ajax({
		...
		success:function (result){
			...
			if (result.code == 1) {
				...
				// 1. 关闭模态框
				$("#myModal").modal("hide");
				// 2. 判断类型ID是否为空（没有Dom操作因为数据库中先添加了，我们刷一下页面才会把数据库中的更新进去）
				if (isEmpty(typeId)) { // 如果为空，则表示执行添加操作的DOM操作
					//参数：typeName、result.result
					//添加操作的result为key
					addDom(typeName, result.result); // 后台返回的主键
				} else { // 如果不为空，则表示执行修改的DOM操作
					//参数：typeName、typeId
					//修改操作result为null，没有装，因为只有一个resultInfo
...
function updatedDom(typeName, typeId) {
	...
	// 1.1. 通过id选择器，得到tr对象
	var tr = $("#tr_" + typeId);
	// 1.2. 修改tr指定单元格的文本值（其中typeName不用加引号）
...
function addDom(typeName, typeId) {
	...
	// 1.2. 通过表格的id属性值，获取表格对象
	var myTable = $("#myTable");

	// 1.3. 判断表格对象是否存在 （长度是否大于0）（表格对象不存在table就不存在了，就无法追加）
	if (myTable.length > 0) { // 如果length大于0，表示表格存在
		// 1.4. 将tr标签追加到表格对象中
		myTable.append(tr);
	} else { // 表示表格不存在就要：先拼接table标签及tr标签，然后再追加
		myTable = '<table id="myTable" class="table table-hover table-striped">';
		myTable += '<tbody> <tr> <th>编号</th> <th>类型</th> <th>操作</th> </tr>';
		myTable += tr + '</tbody></table>';
		// 追加到div中
		//如果为空，顶上有提示信息，myTable += tr
		// 使用myTable.append(tr);直接追加就会有提示信息，但是用html
		// $("#myDiv").html(myTable);就会将“暂未查到数据！”覆盖掉
		$("#myDiv").html(myTable);
	}

	/* 2. 添加左侧类型分组导航栏的列表项  */
	// 2.1. 拼接li元素（typeId是+上去的）
